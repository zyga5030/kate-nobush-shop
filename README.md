...
| `index.html` | The public site GitHub Pages serves | **No — never add admin UI here** |
...
1. Regenerate `index.html` from the working copy with every product baked in.
2. Drop the three files into the repo root: `index.html`, `styles.css`, `avatar.jpg`.
...
The site files live at the repo root, not in `docs/`. An `index.html` at the root
takes precedence over `README.md`, which is what Pages falls back to rendering
when the root has no index page.

`.nojekyll` is required: without it Jekyll strips paths beginning with `_`.
...
- TikTok thumbnail URLs are signed and expire in ~24h, so the page re-fetches them at
  ...
  exception — oembed does not serve profile pictures, so it ships as `avatar.jpg`.
